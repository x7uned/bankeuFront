...
import { fetchAddCard } from "@/redux/slices/cardsSlice";
...
const AddCardForm = () => {
    ...
    const onSubmit = async (data: FormAddCardValues) => {
        try {
            const response = await dispatch(fetchAddCard(data));
            
            if (response.meta.requestStatus === 'fulfilled') {
                toast.success('Card added successfully!');
            } else if (response.meta.requestStatus === 'rejected') {
                const error = response.payload as SerializedError;
                const errorMessage = error.message || 'Error adding card!';
                if (errorMessage.includes('409')) {
                    toast.error('This card already added!');
                } else {
